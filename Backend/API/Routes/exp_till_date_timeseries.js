...
router.get('/:air/exp_till_date',(req,res)=>{
    ...
    let db_query = { _id:0,date:1, 'general.avg_exp_index':1, 'general.avg_imp_index':1 };
    ...
    db.getDB().collection(req.params.air).find({date: {$gte:req.query.sdate,$lt:req.query.edate}, type: req.query.type}).project(db_query).toArray((err,documents)=>{
        if(err){
            ...
            documents.forEach(d=>{
                ...
                expdata.push([new Date(date).getTime(),d.general.avg_exp_index]);
                impdata.push([new Date(date).getTime(),d.general.avg_imp_index])
        ...
        res.status(200).render("chart_template",{option: JSON.stringify(line_chart)});                                    
        }
    })
});
